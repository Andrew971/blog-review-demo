...
const CollectionEmitter = new events_1.default();
var DataType;
(function (DataType) {
    DataType[DataType["S"] = 0] = "S";
    DataType[DataType["N"] = 1] = "N";
})(DataType || (DataType = {}));
class Collection {
    constructor(name = null, schemaDefinition, options = null) {
        this.insertEventCallback = (event) => {
    ...
    putItem(params) {
        try {
            ...
            CollectionEmitter.emit('insert', eventPayload, this.insertEventCallback.bind(this));
            if (this.stream) {
                CollectionEmitter.emit('streamInsert', eventPayload, this.stream);
            }
            const response = {
                Item: [
                    params.Item
    ...
    update(params) {
        try {
            ...
            if (newImage.length > 0) {
                ...
                CollectionEmitter.emit('update', eventPayload, this.updateEventCallback.bind(this));
                if (this.stream) {
                    CollectionEmitter.emit('streamUpdate', eventPayload, this.stream);
                }
                const response = {
    ...
    delete(params) {
        try {
            ...
            CollectionEmitter.emit('remove', eventPayload, this.removeEventCallback.bind(this));
            if (this.stream) {
                CollectionEmitter.emit('streamRemove', streamEventPayload, this.batchSize, this.stream);
            }
            const response = {
                Item: filteredResult
            };
    ...
    updateEventCallback(event) {
        const { NewImage = {}, OldImage = {} } = event.body;
        try {
            if (!NewImage ||
